...
///
/// Steps 21–23 are Track 1's — the page's own look: cover, lock, template.
/// They sit here under different numbers than the ones Track 1 wrote, and
/// deliberately so: Track 1 and Track 3 both numbered forward from 11 in
/// separate trees, so 12–14 were claimed twice for unrelated things. Only one
/// side can keep its numbers, and this is the side that can move — all three
/// go through `add_page_columns` below, which asks `pragma_table_info` before
/// every `ALTER`, so replaying them against a library that already has the
...
"#,
    ...
    // default is the whole migration and a library that predates it opens
    // unchanged.
    //
    // **Why 21.** Track 1 wrote this step as 12 (see `CURRENT_VERSION` above
    // for the whole story): 12–14 were claimed twice, and this side is the one
    // that could move, because `add_page_columns` makes these three replayable
    // where a `CREATE TABLE` would not be.
    sql: "",
    ...
    // existing row, which is the whole migration — a library that predates it
    // opens with nothing locked and nothing dimmed. It is one column on
    // `pages` rather than a flag on each block because the gate that honours
    // it is per page: storing it per block would let a half-locked document
    // exist, and the editor has no way to show that honestly.
    sql: "",
    ...
    // template, so a template is stored as a page whose block sequence is a
    // body to copy from, and this column is the only thing that says so. `0`
    // for every existing row is the whole migration — a library that predates
    // it opens with no templates, and the built-in ones arrive on the next
    // start (see `core::template` and the seeding guard in `app::state`),
    // *not* here: a migration that wrote block rows itself would have to keep
    // the order keys, the `block_children` rows and both FTS indexes in step
...
/// Migration 21 body. Nullable rather than `DEFAULT 0`, so "no cover" and
...
/// Migration 22 body. An integer with a `0` default rather than a nullable:
...
/// Migration 23 body. Shaped exactly like migration 22's for the same reason:
